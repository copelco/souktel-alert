...
<form class="form" action="/xforms/{{ xform.pk }}/edit_field/{{ field.pk }}/" method="post">{% csrf_token %}
{% else %}
<div class="add_field_header">Add New Field</div>
<div class="add_field">
<form class="form" action="/xforms/{{ xform.pk }}/add_field/" method="post">{% csrf_token %}
{% endif %}
...
<td class="field_name_col">
  ...
  {% endif %} {{ form.caption }} {{ form.caption.errors}}</td>
